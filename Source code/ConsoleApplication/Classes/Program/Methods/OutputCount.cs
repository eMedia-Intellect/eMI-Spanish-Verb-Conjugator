...
// <copyright company="eMedia Intellect" file="OutputCount.cs">
...
namespace Emi.SpanishVerbConjugator.ConsoleApplication
{
	...
	public static partial class Program
	{
		private static void OutputCount()
		{
			...
			string[] countProgramModeOptions = new string[] { "--count", "--defective", "--ending", "--inflection" };

			foreach (Option option in options)
			{
				if (!countProgramModeOptions.Contains<string>(option.Name))
				{
					Console.WriteLine(string.Format(CultureInfo.InvariantCulture, "\"{0}\" is not a valid option with the \"--count\" option. See \"--help\" for further information.", option.Name));
					Console.WriteLine();
			...
			else
			{
				...
				Console.WriteLine(defectiveCounter);
			}
